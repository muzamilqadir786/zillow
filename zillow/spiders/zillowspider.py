...
class ZillowspiderSpider(scrapy.Spider):
    ...
    def parse_info(self, response):
        ...
        phone = hxs.xpath('//span[contains(text(),"Property Owner")]/following-sibling::span[@class="snl phone"]/text()').extract()
        print phone
        if phone:
            item['ownerPhone'] = phone[-1]
            print phone

        price = hxs.xpath('normalize-space(//div[@class="main-row  home-summary-row"]/span/text())').extract()
        if price:
            item['price'] = price

        # zestimate = hxs.xpath('//span[conatins(text(),"Zestimate")]/following-sibling::span[1]/text()').extract()
        zestimate = hxs.xpath('normalize-space(//div[@class="  home-summary-row"]/span[2]/text())').extract()
        if zestimate:
            item['zestimate'] = zestimate

        city_state_zip = hxs.xpath('//span[@class="zsg-h2 addr_city"]/text()').extract()
        if city_state_zip:
        ...
        street = hxs.xpath('//header[@class="zsg-content-header addr"]/h1/text()').extract()
        if street:
            item['street'] = street[0].rstrip(', ')


        beds_baths_sqfts = hxs.xpath('//span[@class="addr_bbs"]/text()').extract()
